...
const pt_pin = "{pt_pin}";
const qq = "{qq}";
const qqbotServicePort = "{ServicePort}";
const GOBOT_URL = "{GOBOT_URL}";
const isCheck = "{isCheck}";

// -------系统自动配置部分结束(以上内容勿动)-------

const $ = new Env('京东CK检测');
var cookieArr = [`pt_key=` + pt_key + `;pt_pin=` + pt_pin];
!(async () => {
        ...
        if (cookieArr[i]) {
            ...
            if ($.error) {
                ...
                if ($.isLogin) {
                    if (isCheck != "true") {
                        addCookie();
                        gobotNotify("Cookie提交成功！账户名称：" + $.nickName);
                    }
                }
                else {
                    if (isCheck != "true") {
                        gobotNotify("提交的Cookie无效或已过期。")
                    } else {
                        gobotNotify(`账号🆔：${$.UserName2}已失效！`);
                        DisableCookie();
                    }
                }
            }
...
// 发送通知
function gobotNotify(desp, time = 100) {
    return new Promise((resolve) => {
        if (GOBOT_URL && parseInt(qq) > 10000) {
            const options = {
                url: GOBOT_URL + `/send_private_msg?user_id=` + qq,
                json: {
                    message: `${desp}`
                },
                headers: {
